  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        localStorage.setItem('username', form.username); // Store token in localStorage
        navigate('/manager');
      }
    } catch (error) {
    ...
    <>
      ...
      <div className="flex justify-center items-center min-h-screen bg-purple-100">
        ...
        <h2 className="text-2xl font-bold mb-4 text-center text-purple-900">
          ...
          <button className="w-1/2 p-2 bg-purple-700  text-white rounded " onClick={()=>{navigate("/resetPassword")}}> Reset Password </button>
        </form>
